    ...
    <form action="index.php?type=dethi&trang=1" method="POST">
        <div class="mt-5 d-flex justify-content-center " id="search">
            <div class="input-group w-50">
                ...
                <button type="submit" class="btn btn-outline-light " name="btn-search" data-mdb-ripple-init >SEARCH</button>
            </div>
        </div>
    </form>
...
<div class="bg-light d-flex align-items-center" style="flex-direction:column;padding-bottom:150px; min-height: 500px;">
    ...
    foreach ($getAllMonHoc as $key) {
        echo '
        <div class="list-group mt-5 " style="width:90%">
            ...
            foreach ($getTop5DeMon as $key1) {
                if ($i == 0) {
                    echo '
                        <a href="index.php?type=cate&id=' . $key1['id_dethi'] . '" 
                            class="list-group-item list-group-item-action active" style="background-color: #155E94" aria-current="true">
                        ' . $i . '.' . $key1['tenDeThi'] . '
                        </a>
                    ';
                } else {
